...
В основе проекта лежат два класса, которые описывают сущности интернет-магазина.

### 📦 Класс `Product`
Товар с конкретными характеристиками:
- `name` — название
- `description` — описание
- `price` — цена (с копейками, тип `float`)
- `quantity` — сколько штук в наличии

```python
from src.products import Product

phone = Product("Смартфон", "AMOLED, 128GB", 49999.0, 10)
print(phone.name)  # Смартфон
```
### 📂 Класс Category
Категория, в которой собраны товары:
...
category = Category("Электроника", "Гаджеты и устройства", [phone, tablet])
print(category.name)          # Электроника
print(len(category.products)) # 2
```
### 📊 Автоматические счётчики
У класса Category есть два атрибута, которые обновляются сами:

- Category_count — сколько всего категорий создано

- Product_count — сколько всего товаров во всех категориях

Каждый раз, когда ты создаёшь новую категорию, счётчики увеличиваются автоматически. Удобно и без лишнего кода.
```python
from src.products import Product, Category
...
cat1 = Category("Кат1", "Описание кат1", [p1])
cat2 = Category("Кат2", "Описание кат2", [p2, p1])

print(Category.category_count) # 2
print(Category.product_count)  # 3 (1 + 2)
```
✅ Тесты

- Всё покрыто тестами (pytest):

- Проверяется создание продуктов и категорий

- Проверяется работа счётчиков

- Тестируется загрузка из JSON (доп. задание)

```bash
poetry run pytest -v
```
### ⭐ Дополнительное задание
Реализована функция load_products_from_json, которая читает файл data/products.json 
и превращает его в живые объекты Product и Category. Если файла нет — просто возвращает пустой список, без ошибок.
...
```
Всё работает, всё красиво. Дальше — только развивать! 😎
